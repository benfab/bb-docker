...
# Wait for SSH 
function check_ssh_manager {
  for i in $(seq 1 $NBR_MANAGER); do
  status=nok    
  count=0
	while [ "$status" != "Docker" ] && [ "$count" -lt 150 ]; do
		 status=$(docker-machine ssh ${MANAGER}$i docker --version 2>/dev/null | awk '{print $1}' )
                 count=$((count+1))
		 sleep 2
	done
  done
...
function check_ssh_worker {
  for i in $(seq 1 $NBR_WORKER); do
  status=nok
  count=0
        while [ "$status" != "Docker" ] && [ "$count" -lt 150 ]; do
                 status=$(docker-machine ssh ${WORKER}$i docker --version 2>/dev/null | awk '{print $1}' )
		 count=$((count+1))
                 sleep 2
        done
  done
